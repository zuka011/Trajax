...
class test_that_covariance_is_propagated:
    ...
    def cases(create_propagator, data) -> Sequence[tuple]:
        return [
            (  # No input uncertainty -> position covariance remains constant
            ...
            (  # Linear growth: p + t * dt² * v
                propagator := create_propagator.linear(
                    time_step_size=2.0,
                    covariance=create_propagator.covariance.composite(
                        state_provider=create_propagator.covariance.constant_variance(
                            variance=3.0, dimension=2
                        ),
                        input_provider=create_propagator.covariance.constant_variance(
                            variance=1.5, dimension=2
                        ),
                    ),
                ),
                states := data.obstacle_2d_poses(
            ...
            (  # Multiple obstacles with correlation.
                propagator := create_propagator.linear(
                    ...
                    covariance=create_propagator.covariance.composite(
                        state_provider=create_propagator.covariance.constant_covariance(
                            covariance=array(
                                [
                        ...
                        input_provider=create_propagator.covariance.constant_covariance(
                            covariance=array(
                                [
                                    [
                                        [vx0 := 1.0, vx1 := 2.0],
            ...
            (  # Single time step
                propagator := create_propagator.linear(
                    ...
                    covariance=create_propagator.covariance.composite(
                        state_provider=create_propagator.covariance.constant_variance(
                            variance=(p := 0.0), dimension=2
                        ),
                        input_provider=create_propagator.covariance.constant_variance(
                            variance=(v := 2.0), dimension=2
                        ),
                    ),
                ),
                states := data.obstacle_2d_poses(
